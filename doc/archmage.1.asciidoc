...
archmage - Binary space partitioning window manager

Synopsis
--------

*archmage* [*-h*|*-v*|*-c* 'CONFIG_PATH']

*archmagec* 'DOMAIN' ['SELECTOR'] 'COMMANDS'

*archmagec* 'COMMAND' ['OPTIONS'] ['ARGUMENTS']

Description
-----------

*archmage* is a tiling window manager that represents windows as the leaves of a full binary tree.

It is controlled and configured via *archmagec*.
...
*-c*, *--count* 'COUNT'::
	Stop the corresponding *archmagec* process after having received 'COUNT' events.
...
If the server can't handle a message, *archmagec* will return with a non-zero exit code.
...
Please note that *archmage* initializes monitors before it reads messages on its socket, therefore the initial monitor events can't be received.
...
'ARCHMAGE_SOCKET'::
	The path of the socket used for the communication between *archmagec* and *archmage*. If it isn't defined, then the following path is used: '/tmp/archmage<host_name>_<display_number>_<screen_number>-socket'.
